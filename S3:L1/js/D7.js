...
function concStr(str1, str2) {
  str1 = str1.slice(0, 2);
  str2 = str2.slice(-3);
  resConc = str1.concat(str2);
}
concStr("Lorem", "Ipsum");
console.log(resConc.toUpperCase());
...
function newArr(){
  for (let i = 0; i <= 9; i++) {
    arr10[i] = arr10.push(Math.random()*101)
  }
}
newArr();
console.log(arr10);

/*function randArr(n) // n-> numero degli elementi
const arr = [];
...
let resPari =[];
function numPari() {
  ///arrPari.forEach((element) => num % 2 === 0);

  resPari = arrPari.filter (num => num % 2 === 0);
}
numPari(5,4,7);
console.log(resPari);
...
let sum = 0
function sumArr() {
  somArr.forEach((element) => sum += element);
  };
sumArr(8,3,5);
console.log(sum);
...
let sum1 = 0
function sumArr1() {
  somArr1.reduce((element) => sum1 += element, 0);
  };
sumArr1(8,3,5);
console.log(sum1);
...
console.log(arrStrFunc("Epicode", "JavaScript", "Fullstack", "abc", "lunedi"))
 */
